...
function LeftContainer(props) {
  ...
  const handleAddNewList = async () => {
    ...
    try {
      ...
      initTodoList._id = response.data.todoId;
  ...
  function RenderTodoLists({props}) {
    ...
    const handleNameSave = async () => {
      const updatedName = todoListNameRef.current.value;
      
      
      // Send request to backend to save name change
      try { 
        const response = await axios.post("http://localhost:3000/api/updateTodoList", {
          name: updatedName,
